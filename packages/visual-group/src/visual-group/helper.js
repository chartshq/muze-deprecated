...
export const createMatrices = (context) => {
    ...
    const encoders = getEncoders(layers);
    resolver.encoder(encoders.planarEncoder);

    // Set the group type
    context.groupType(encoders.planarEncoder.constructor.type());

    // Get sanitized fields as instances of the Vars Class
    const fields = encoders.planarEncoder.fieldSanitizer(datamodel, matrixConfig);
    encoders.planarEncoder.setAxisAndHeaders(config ? config.axisFrom : {}, fields);
